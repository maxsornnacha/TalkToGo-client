...
export default function Postform(props){
    ...
    <div>
        <div> 
            ...
            <div className="w-full flex flex-col items-center h-screen md:h-60 lg:h-60  overflow-auto"> 
            <div className="w-full mt-5 px-5 ">
                <textarea value={content} onChange={(event)=>setContent(event.target.value)}  className={content && content.length>150?"w-full outline-none text-[0.9rem] ":"w-full outline-none text-[1.2rem] "} rows={7} placeholder={`คุณคิดอะไรอยู่ ${props.accountData.firstname}`}/>
            </div>
