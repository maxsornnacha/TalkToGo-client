...
export default function AllFriendList({accountID,friendDataUpdate,accountLogin,dataFiltered}){
    ...
    <div className="md:grid md:grid-cols-12 gap-5 mt-5 lg:p-2 md:p-2 w-full flex flex-col p-3 ">
        ...
        <div key={index} className="border border-gray-200 xl:col-span-4 2xl:col-span-3  md:col-span-6  bg-white rounded-md ">
             {/* responsive สำหรับหน้าจอขนาดกลางขึ้นไป */}
            ...
            <div className="flex justify-center p-5 hidden md:block"><AddFriends accountData={friendAccountData} index={index} handleChatroomToggle={handleChatroomToggle} senderID={accountLogin.accountData._id} getterID={friendAccountData._id}/></div>
            }
         
            
             {/* responsive สำหรับหน้าจอขนาดเล็ก */}
            <div className="flex justify-between md:hidden h-auto">
            <Link href={`/profile/${friendAccountData.id}`} className="flex items-center w-5/6 py-1 ">
                <img src={friendAccountData.accountImage} className="w-20 h-20 rounded-full ms-3" />
                <div>
            ...
            {!(accountLogin.accountData._id === friendAccountData._id) &&
            <div className="flex justify-center w-36 "><AddFriends accountData={friendAccountData} index={index} handleChatroomToggle={handleChatroomToggle} senderID={accountLogin.accountData._id} getterID={friendAccountData._id}/></div>
            }
            </div>
